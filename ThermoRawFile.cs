...
namespace CSMSL.IO.Thermo
{
    public class ThermoRawFile : MSDataFile<ThermoSpectrum>
    {
        ...
        public Tuple<double, string[,]> GetStatusLogForScanNum(int nScanNumber, List<double> writtenRetentionTimes)
        {
            double pdStatusLogRT = 0;
            object pvarLabels = null;
            object pvarValues = null;
            int pnArraySize = 0;

            _rawConnection.GetStatusLogForScanNum(nScanNumber, ref pdStatusLogRT, ref pvarLabels, ref pvarValues, ref pnArraySize);

            if (writtenRetentionTimes.Count > 0 && writtenRetentionTimes.Last() == pdStatusLogRT)
            {
                return null;
            }
            else
            {
                writtenRetentionTimes.Add(pdStatusLogRT);
            }

            var values = (string[]) pvarValues;
            var labels = (string[]) pvarLabels;
            var returnArray = new string[2, pnArraySize];
            for (int i = 0; i < pnArraySize; i++)
            {
                returnArray[0, i] = labels[i];
                returnArray[1, i] = values[i];
            }
            return new Tuple<double, string[,]>(pdStatusLogRT, returnArray);
        }
        ...
        public DateTime GetCreationDate()
        {
            ...
            _rawConnection.GetCreationDate(ref pCreationDate);
        ...
        {
            string objectFlag = flag;
            object pvarValue = null;

            _rawConnection.GetTrailerExtraValueForScanNum(spectrumNumber, objectFlag, ref pvarValue);
